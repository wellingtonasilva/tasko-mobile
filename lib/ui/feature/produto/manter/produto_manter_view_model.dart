...
class ProdutoManterViewModel extends Notifier<ProdutoManterUiState> {
  ...
  ProdutoManterUiState build() {
    return ProdutoManterUiState(
      obterPorIdCommand: Command1<ProdutoResponse, (int id,)>(_obterPorId),
      atualizarCommand:
          Command1<ProdutoResponse, (int id, AtualizarProdutoRequest request)>(
            _atualizar,
          ),
      listarUnidadeMedidaCommand: Command0<void>(_listarProdutoUnidadeMedida)
        ..execute(),
      listarGrupoCommand: Command0<void>(_listarProdutoGrupo)..execute(),
      listarSubgrupoCommand: Command0<void>(_listarProdutoSubgrupo)..execute(),
    );
  }
  ...
  Future<Result<ProdutoResponse>> _obterPorId((int id,) parameters) async {
    onStartEvent?.call();
    final (id,) = parameters;
    ...
    if (result is Success<ProdutoResponse>) {
      state = state.copyWith(produto: result.value, produtoDraft: result.value);
  ...
  Future<Result<ProdutoResponse>> _atualizar(
    (int id, AtualizarProdutoRequest request) parameters,
  ) async {
    onStartEvent?.call();
    ...
    if (result is Success<ProdutoResponse>) {
      state = state.copyWith(produto: null, produtoDraft: null);
...
final produtoManterViewModelProvider =
    NotifierProvider.autoDispose<ProdutoManterViewModel, ProdutoManterUiState>(
