...
import 'package:tasko_mobile/ui/feature/produto/adicionar/dados_basicos/produto_adicionar_dados_basicos_controllers.dart';
...
class ProdutoAdicionarDadosBasicosScreen extends BaseScreen {
  final Function(String value) onPrevious;
  final Function(String value) onNext;

  const ProdutoAdicionarDadosBasicosScreen({
    super.key,
  ...
  @override
  BaseScreenState<ProdutoAdicionarDadosBasicosScreen> createState() =>
      _ProdutoAdicionarDadosBasicosScreenState();
}

class _ProdutoAdicionarDadosBasicosScreenState
    extends BaseScreenState<ProdutoAdicionarDadosBasicosScreen> {
  late final ProdutoAdicionarDadosBasicosControllers _controllers;
  String? _hydratedDraftKey;

  @override
  void initState() {
    super.initState();
    _controllers = ProdutoAdicionarDadosBasicosControllers();
  ...
  Widget _buildLoadingDropdownFieldGrupo(ProdutoAdicionarUiState viewModel) {
    final notifier = ref.read(produtoAdicionarViewModelProvider.notifier);
  ...
  Widget buildDropdownFieldGrupo(ProdutoAdicionarUiState viewModel) {
    final notifier = ref.read(produtoAdicionarViewModelProvider.notifier);
    final selectedGrupo =
  ...
  Widget _buildLoadingDropdownFieldSubgrupo(ProdutoAdicionarUiState viewModel) {
    final notifier = ref.read(produtoAdicionarViewModelProvider.notifier);
  ...
  Widget _buildDropdownFieldSubgrupo(ProdutoAdicionarUiState viewModel) {
    final notifier = ref.read(produtoAdicionarViewModelProvider.notifier);
  ...
  Widget _buildLoadingDropdownFieldUnidadeMedida(
    ProdutoAdicionarUiState viewModel,
  ) {
    final notifier = ref.read(produtoAdicionarViewModelProvider.notifier);
  ...
  Widget _buildDropdownFieldUnidadeMedida(ProdutoAdicionarUiState viewModel) {
    final notifier = ref.read(produtoAdicionarViewModelProvider.notifier);
